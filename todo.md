...
and each exclusion in `.clang-tidy` carries the reason it was measured to be noise. **85 is the
...
Most of the residue is one analyzer artifact: the game's own defensive `(tr.ent) &&` /
`(tr.surface) &&` checks teach the analyzer a nullability the engine's trace contract rules out.
Two are known false and need no second look: `bugprone-inc-dec-in-conditions` on the
